  ...
  try {
    ...
    const [camps, total] = await Promise.all([
      BloodCamp.find(filter)
        .populate("hospital", "name facilityType address")
        .sort({ date: 1 })
        .skip(skip)
    ...
    const mappedCamps = camps.map((camp) => {
      const existingRegistration = (camp.registeredDonors || []).find((reg) =>
        reg.donor?.toString() === donorId?.toString(),
      );
  ...
  try {
    ...
    if (camp.status === "Cancelled" || camp.status === "Completed") {
      return res.status(400).json({
        success: false,
        message: "Donations cannot be registered for cancelled or completed camps.",
      });
    }
    ...
    res.status(200).json({
      success: true,
      message: "Donation request sent to the camp.",
      data: { campId: camp._id, registrationStatus: "Requested" },
    });
  } catch (error) {
    console.error("Register For Camp Error:", error);
    res.status(500).json({
      success: false,
      message: "Failed to register for the donation camp.",
      error: error.message,
    });
  }
};
...
export const completeDonation = async (req, res) => {
  try {
    ...
    const camp = await BloodCamp.findById(campId).populate("hospital");
    if (!camp) {
      return res.status(404).json({ success: false, message: "Camp not found" });
    }
    ...
    if (camp.status === "Cancelled") {
      return res.status(400).json({
        success: false,
        message: "Cannot complete donation for a cancelled camp.",
      });
    }

    const registration = (camp.registeredDonors || []).find((reg) =>
      reg.donor?.toString() === donorId.toString(),
    );
    ...
    if (!registration) {
      return res.status(400).json({
        success: false,
        message: "You must register for the camp before marking the donation complete.",
      });
    }
    ...
    if (registration.status === "Completed") {
      return res.status(400).json({
        success: false,
        message: "This donation has already been marked as completed.",
      });
    }
    ...
    if (camp.expectedDonors > 0 && camp.actualDonors >= camp.expectedDonors) {
    ...
    donor.donationHistory.push({
      ...
      bloodGroup: donor.bloodGroup,
      quantity: 1,
      remarks: `Donation completed at ${camp.title}`,
      verified: true,
    });
    ...
    if (camp.hospital?.facilityType === "hospital") {
      bloodData.hospital = camp.hospital._id;
    } else {
      bloodData.bloodLab = camp.hospital?._id;
    }

    let bloodRecord = await Blood.findOne({
      bloodGroup: donor.bloodGroup,
      ...(bloodData.bloodLab ? { bloodLab: bloodData.bloodLab } : {}),
      ...(bloodData.hospital ? { hospital: bloodData.hospital } : {}),
      expiryDate: { $gt: new Date() },
    });

    if (bloodRecord) {
      bloodRecord.quantity += 1;
      await bloodRecord.save();
    } else {
      await Blood.create(bloodData);
    }

    if (camp.hospital) {
      const facility = await Facility.findById(camp.hospital._id);
      if (facility) {
        facility.history.push({
          eventType: "Donation",
          description: `Donor ${donor.fullName} completed donation at camp ${camp.title}.`,
          date: new Date(),
        });
        await facility.save();
      }
    ...
    res.status(200).json({
      success: true,
      message: "Donation completed successfully. Your history and blood store were updated.",
      data: {
        campId: camp._id,
    ...
    res.status(500).json({
      success: false,
      message: "Failed to complete the donation.",
      error: error.message,
    });
  }
};
