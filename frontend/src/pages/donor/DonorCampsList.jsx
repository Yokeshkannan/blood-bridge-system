...
const CampCard = ({ camp, onRegister, onComplete, registering, completing }) => {
  const isCompleted = camp.status === 'Completed';
  const isCancelled = camp.status === 'Cancelled';
  const isUpcoming = camp.status === 'Upcoming';
  // const isOngoing = camp.status === 'Ongoing';

  const registrationStatus = camp.registrationStatus || null;
  const isRegistered = Boolean(registrationStatus);
  const isDonationCompleted = registrationStatus === 'Completed';
  const canRegister = !isRegistered && !isCancelled && !isCompleted;
  const canCompleteDonation = isRegistered && !isDonationCompleted && !isCancelled;
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3 items-start">
          {canRegister && (
            <button
              ...
              className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-red-600 text-white hover:bg-red-700 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          {canCompleteDonation && (
            <button
              onClick={() => onComplete(camp._id)}
              disabled={completing}
              className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {completing ? 'Completing...' : 'Mark as Donated'}
            </button>
          )}

          {isDonationCompleted && (
            <span className="inline-flex items-center px-3 py-2 rounded-full bg-emerald-100 text-emerald-700 text-sm font-semibold">
              Donation Completed
            </span>
          )}

          {isRegistered && !isDonationCompleted && !canCompleteDonation && (
            <span className="text-sm text-gray-600">Registration pending</span>
          )}
        </div>
      </div>
    </div>
...
export const DonorCampsList = () => {
  ...
  const [completingCampId, setCompletingCampId] = useState(null);
  ...
  const registerForCamp = async (campId) => {
    const token = localStorage.getItem("token");
    if (!token) {
      toast.error("Please log in to register for a camp.");
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/donor/camps/${campId}/register`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      fetchCamps();
      window.dispatchEvent(new Event("donorDataUpdated"));
    } catch (err) {
      console.error("Register For Camp Error:", err);
      const message = err.response?.data?.message || "Failed to register for the camp.";
      toast.error(message);
    } finally {
  ...
  const completeDonation = async (campId) => {
    const token = localStorage.getItem("token");
    if (!token) {
      toast.error("Please log in to complete the donation.");
      return;
    }

    setCompletingCampId(campId);
    try {
      const response = await axios.post(
        `${API_BASE_URL}/donor/camps/${campId}/complete`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      toast.success(response.data.message || "Donation marked complete.");
      fetchCamps();
      window.dispatchEvent(new Event("donorDataUpdated"));
    } catch (err) {
      console.error("Complete Donation Error:", err);
      const message = err.response?.data?.message || "Failed to complete the donation.";
      toast.error(message);
    } finally {
      setCompletingCampId(null);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
              {displayedCamps.map((camp) => (
                <CampCard
                  key={camp._id}
                  camp={camp}
                  onRegister={registerForCamp}
                  onComplete={completeDonation}
                  registering={registeringCampId === camp._id}
                  completing={completingCampId === camp._id}
                />
              ))}
            </div>
